...
const AllPatientsSpeech = () => {
  ...
  const fetchSpeechPatients = async () => {
    ...
    try {
      const response = await axiosInstance.get(
        `/authentication/speech-assignments?page=${currentPage}&search=${search}`,
      )
      ...
      // Make sure each assignment has a valid patient object
      const validAssignments = assignmentsData.filter(
        (assignment) => assignment && assignment.patient && typeof assignment.patient === "object",
      )

      setAssignments(validAssignments)

      // Set total pages from response or default to 1
      setTotalPages(response.data.totalPages || 1)
    } catch (error) {
      console.error("Error fetching speech patients:", error)
  ...
  // Safe getter function for patient properties
    ...
    <div className="card h-100 p-0 radius-12">
      <div className="card-header border-bottom bg-base py-16 px-24 d-flex align-items-center flex-wrap gap-3 justify-content-between">
        <div className="d-flex align-items-center flex-wrap gap-3">
          <h6 className="text-lg mb-0">Speech Department Patients</h6>
        ...
        <button
          onClick={() => router.push("/speech-department/assign-patients")}
      ...
      <div className="card-body p-24">
        {loading ? (
          ...
          <div className="text-center py-4">
            <p className="text-muted">No patients assigned to speech department</p>
            <button onClick={() => router.push("/speech-department/assign-patients")} className="btn btn-primary mt-2">
              Assign Patients
            </button>
          </div>
        ) : (
                  ...
                  <tr key={assignment._id || index}>
                    ...
                    <td className="text-center">
                      <div className="d-flex align-items-center gap-10 justify-content-center">
                        <button
                          onClick={() => router.push(`/speech-department/plan/${assignment.patient?._id}`)}
                          type="button"
                        ...
                        <button
                          onClick={() => router.push(`/speech-department/test/${assignment.patient?._id}`)}
                          type="button"
        ...
        <div className="d-flex align-items-center justify-content-between flex-wrap gap-2 mt-24">
          <span className="text-gray-900">Showing {assignments.length} speech department patients</span>
