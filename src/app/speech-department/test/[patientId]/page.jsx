...
const PatientTestEditor = () => {
  ...
  const fetchExistingExam = async () => {
    try {
      const response = await axiosInstance.get(`${process.env.NEXT_PUBLIC_API_URL}/authentication/speech-exam/${patientId}`)
      if (response.data) {
        setExam(response.data)
      }
    } catch (error) {
      console.error("No existing exam found or error:", error)
    }
  }
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      let response
      if (exam._id) {
        response = await axiosInstance.put(`${process.env.NEXT_PUBLIC_API_URL}/authentication/speech-exam/${exam._id}`, examData)
      } else {
        response = await axiosInstance.post(`${process.env.NEXT_PUBLIC_API_URL}/authentication/speech-exam`, examData)
      }
  ...
  const handleDownloadDocument = () => {
    if (exam.filePath) {
      window.open(`${process.env.NEXT_PUBLIC_API_URL}/authentication/download-document/${exam.filePath}`, "_blank")
    }
  }
  ...
  const getDocumentViewerUrl = () => {
    ...
    const documentUrl = `${process.env.NEXT_PUBLIC_API_URL}/authentication/view-document/${exam.filePath}`
        ...
        <div className="col-12">
          <div className="card">
            <div className="card-header d-flex justify-content-between align-items-center">
              <div>
                <h5 className="mb-0">Speech Test - {patient.name}</h5>
            ...
            <div className="card-body">
              ...
              <div className="row mb-3">
                ...
                <div className="col-4">
                  ...
                  <input
                    ...
                    className="form-control"
                    value={exam.score || ""}
                    onChange={(e) => setExam((prev) => ({ ...prev, score: Number(e.target.value) }))}
              ...
              <div className="row">
                <div className="col-12">
                  <label className="form-label">Test Document Viewer</label>
                  ...
                  {exam.filePath ? (
                    <div className="border rounded bg-light" style={{ height: "700px", overflow: "hidden" }}>
                      <div className="d-flex justify-content-between align-items-center p-2 bg-white border-bottom">
                        <div className="d-flex align-items-center gap-2">
                          ...
                          <span className="small fw-medium">{exam.fileName || "Document"}</span>
                        </div>
                    ...
                    >
                      ...
                      <h4 className="mt-3 mb-2">No Test Document Uploaded</h4>
              ...
              {exam.lastModified && (
                <div className="row mt-3">
                  <div className="col-12">
                    <small className="text-muted">Last modified: {new Date(exam.lastModified).toLocaleString()}</small>
                  </div>
                </div>
              )}
            </div>
...
export default PatientTestEditor
