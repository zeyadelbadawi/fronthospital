...
const AssignPatientsToOccupationalTherapy = () => {
  ...
  const fetchAssignedPatients = async () => {
    try {
      const response = await axiosInstance.get(`${process.env.NEXT_PUBLIC_API_URL}/OccupationalTherapy/Occupational-therapy-assignments`)
  ...
  const handleAssignPatient = async (patientId) => {
    ...
    try {
      await axiosInstance.post(`${process.env.NEXT_PUBLIC_API_URL}/OccupationalTherapy/assign-to-Occupational`, { patientId })
      setAssignedPatients([...assignedPatients, patientId])
      alert("Patient assigned to occupational therapy department successfully!")
    } catch (error) {
      console.error("Error assigning patient:", error)
      alert("Error assigning patient to occupational therapy department")
    }
  }

  const handleUnassignPatient = async (patientId) => {
    try {
      await axiosInstance.delete(`${process.env.NEXT_PUBLIC_API_URL}/OccupationalTherapy/unassign-from-Occupational/${patientId}`)
      setAssignedPatients(assignedPatients.filter((id) => id !== patientId))
      alert("Patient unassigned from occupational therapy department successfully!")
    } catch (error) {
      console.error("Error unassigning patient:", error)
      alert("Error unassigning patient from occupational therapy department")
    }
  }
    ...
    <div className="card h-100 p-0 radius-12">
      <div className="card-header border-bottom bg-base py-16 px-24 d-flex align-items-center flex-wrap gap-3 justify-content-between">
        <div className="d-flex align-items-center flex-wrap gap-3">
          <h6 className="text-lg mb-0">Assign Patients to Occupational Therapy Department</h6>
        ...
        <button
          onClick={() => router.push("/occupational-therapy/patients")}
          className="btn btn-primary-600 radius-8 px-20 py-11"
        >
          View Occupational Therapy Patients
        </button>
      </div>
        ...
        <td className="text-center">
          {assignedPatients.includes(patient._id) ? (
            <button
              ...
              title="Unassign from Occupational Therapy Department"
            >
            ...
            <button
              ...
              title="Assign to Occupational Therapy Department"
            >
...
export default AssignPatientsToOccupationalTherapy
