...
const PatientTestEditor = () => {
  ...
  const fetchExistingExam = async () => {
    try {
      const response = await axiosInstance.get(`${process.env.NEXT_PUBLIC_API_URL}/authentication/physical-therapy/exam/${patientId}`)
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const response = await axiosInstance.post(`${process.env.NEXT_PUBLIC_API_URL}/authentication/physical-therapy/exam`, ExamData)
      setExam(response.data)
      alert("Exam saved successfully!")
    } catch (error) {
      console.error("Error saving Exam:", error)
      alert("Error saving Exam: " + (error.response?.data?.message || error.message))
    } finally {
      setSaving(false)
    }
  }
  ...
  const handleFileUpload = async (event) => {
    ...
    try {
      ...
      const response = await axiosInstance.post(`${process.env.NEXT_PUBLIC_API_URL}/authentication/py/upload-test`, formData, {
  ...
  const handleDownloadDocument = () => {
    if (Exam.filePath) {
      window.open(`${process.env.NEXT_PUBLIC_API_URL}/physical-therapy/download-Exam/${Exam.filePath}`, "_blank")
    }
  }
        ...
        <div className="col-12">
          <div className="card">
            <div className="card-header d-flex justify-content-between align-items-center">
              <div>
                <h5 className="mb-0">Physical Therapy Exam - {patient.name}</h5>
