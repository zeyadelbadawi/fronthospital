...
export function AppointmentAssignmentManager() {
    ...
    const fetchAvailableDoctors = async (departments) => {
        setLoadingDoctors(true)
        try {
            console.log("[v0] Fetching doctors for department:", departments)

            const response = await axiosInstance.get(
                `${process.env.NEXT_PUBLIC_API_URL}/authentication/doctors?department=${encodeURIComponent(departments)}`,
            )

            let doctors = response.data.doctors || []

            console.log(
                "[v0] All doctors from API:",
                doctors.map((doc) => ({
                    id: doc._id,
            ...
            doctors = doctors.filter((doc) => {
                if (!doc.department) {
                    console.log("[v0] Doctor has no department:", doc.username)
                    return false
                }
                const docDepartments = Array.isArray(doc.department) ? doc.department : [doc.department]

                console.log("[v0] Comparing doctor departments:", {
                    docDepartments,
                    filterDept: department,
                    match: docDepartments.some((dept) => dept.toLowerCase().trim() === department.toLowerCase().trim()),
                })

                return docDepartments.some((dept) => dept.toLowerCase().trim() === department.toLowerCase().trim())
            })
            ...
            // If there's an existing assignment, exclude that doctor from the list
            if (assignModal.appointment?.assignmentDepartment) {
                const existingAssignment =
                    assignments[`${assignModal.appointment._id}-${assignModal.appointment.assignmentDepartment}`]
    ...
    const handleOpenAssignModal = async (appointment, department) => {
        ...
        await fetchAvailableDoctors(department)
    }
    ...
    const confirmAssignment = async () => {
        ...
        try {
            ...
            // Check if there's an existing assignment to update or create new
