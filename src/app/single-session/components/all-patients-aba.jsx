...
import { Search, ClipboardList, Users, Brain, Calendar, Phone, Mail, User, X, Eye, CheckCircle, AlertCircle, FileText, ChevronLeft, Filter, Clock, CheckSquare, XCircle } from 'lucide-react'
...
const AllPatientAba = () => {
  ...
  const fetchAbaPatients = async () => {
    ...
    try {
      ...
      const response = await axiosInstance.get(`${process.env.NEXT_PUBLIC_API_URL}/abaS/ABA-assignments?${params}`)

      const assignmentsData = Array.isArray(response.data) ? response.data : response.data.assignments || []
      setAssignments(assignmentsData)
      setTotalPages(response.data.totalPages || 1)
    } catch (error) {
      console.error("Error fetching ABA therapy Students:", error)
      setAssignments([])
    } finally {
      setLoading(false)
    }
  ...
  const handleCompleteAssignment = async () => {
    ...
    try {
      const response = await axiosInstance.put(
        `${process.env.NEXT_PUBLIC_API_URL}/abaS/complete-assignment/${completeModal.assignment._id}`,
  ...
  const handlePlanClick = (patientId) => {
    router.push(`/ABAS/plan/${patientId}`)
  }
        ...
        <div className={styles.cardHeader}>
          <div className={styles.headerContent}>
            <div className={styles.headerLeft}>
              ...
              <h2 className={styles.pageTitle}>ABA Therapy Students</h2>
              <p className={styles.pageSubtitle}>Manage and view all students assigned to ABA</p>
            </div>
        ...
        <div className={styles.cardBody}>
          {loading ? (
            <div className={styles.loadingContainer}>
              ...
              <p className={styles.loadingText}>Loading ABA Students...</p>
            </div>
          ) : assignments.length === 0 ? (
            <div className={styles.noData}>
              <div className={styles.emptyState}>
                <Brain className={styles.emptyIcon} />
                <h3>No ABA Students Found</h3>
                <p>
                  {search || statusFilter
                    ? "No students match your search criteria. Try adjusting your filters."
                    : "No students are currently assigned to the ABA department."}
                </p>
              </div>
            </div>
          ) : (
          ...
          <div className={styles.modal}>
            ...
            <div className={styles.modalContent}>
              <div className={styles.confirmationMessage}>
                ...
                <p>
                  Are you sure you want to mark the ABA assignment for{" "}
