...
const AssignPatientsToSpecialEducation = () => {
  ...
  const fetchAssignedPatients = async () => {
    try {
      const response = await axiosInstance.get(
        `${process.env.NEXT_PUBLIC_API_URL}/SpecialEducation/Special-Education-assignments`,
      )
  ...
  const handleAssignPatient = async (patientId) => {
    ...
    try {
      ...
      alert("Patient assigned to Special Education department successfully!")
    } catch (error) {
      console.error("Error assigning patient:", error)
      alert("Error assigning patient to Special Education department")
    }
  }
  ...
  const handleUnassignPatient = async (patientId) => {
    try {
      await axiosInstance.delete(
        `${process.env.NEXT_PUBLIC_API_URL}/SpecialEducation/unassign-from-Special-Education/${patientId}`,
      )
      setAssignedPatients(assignedPatients.filter((id) => id !== patientId))
      alert("Patient unassigned from Special Education department successfully!")
    } catch (error) {
      console.error("Error unassigning patient:", error)
      alert("Error unassigning patient from Special Education department")
    }
  }
      ...
      <div className={styles.assignPatientsCard}>
        <div className={styles.cardHeader}>
          ...
          <button onClick={() => router.push("/physical-therapy/patients")} className={styles.primaryButton}>
            View Special Education Patients
          </button>
        </div>
                        ...
                        <td className={styles.textCenter}>
                          {assignedPatients.includes(patient._id) ? (
                            ...
                            <button
                              ...
                              title="Assign to Specialc Education Department"
                            >
