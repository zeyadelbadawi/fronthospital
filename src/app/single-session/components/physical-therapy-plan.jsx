...
const PhysicalTherapyPlan = () => {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const response = await axiosInstance.post(`${process.env.NEXT_PUBLIC_API_URL}/physicalTherapy/plan`, planData)
  ...
  if (!patient) {
    return <div className={styles.loadingContainer}>Loading patient data...</div>
  }
        ...
        <div className={styles.col12}>
          <div className={styles.card}>
            <div className={styles.cardHeader}>
              <div>
                <h5 className={styles.cardTitle}>Physical Therapy Plan - {patient.name}</h5>
                <small className={styles.patientId}>Patient ID: {patient._id}</small>
              </div>
            </div>
                ...
                <div className={styles.col12}>
                  ...
                  <SyncfusionDocx
                    userData={{
                      docxId: plan._id,
                      patientId,
                      filePath: `${process.env.NEXT_PUBLIC_API_URL}/uploads/physical-therapy/plan/${
