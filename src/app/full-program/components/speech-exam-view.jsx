...
import DocxUploadFormSpeechExam from "./docx-upload-form-speech-exam"
import DocxUploadFormSpeechExamPrevious from "./docx-upload-form-speech-exam-previous" // New import
import CloseQuarterFormSpeechExam from "./close-quarter-form-speech-exam.jsx"
...
export default function SpeechExamView({ patientId, onBack }) {
  ...
  const determineActiveQuarter = async () => {
    ...
    try {
      const response = await axiosInstance.get(
        `${process.env.NEXT_PUBLIC_API_URL}/speech/get-exams/${patientId}?last=true`,
      )
  ...
  const fetchAllHistoricalExams = useCallback(async () => {
    ...
    setError(null)
    try {
      const response = await axiosInstance.get(`${process.env.NEXT_PUBLIC_API_URL}/speech/exams/${patientId}`)
  ...
  const handleSaveContent = async (documentContentBlob) => {
    ...
    try {
      ...
      formData.append("title", selectedExam.title || `Speech Exam for ${patient?.name || patientId}`)
            ...
            <div className={styles.planEditorCard}>
              ...
              <div className={styles.planBody}>
                <SyncfusionDocx
                  ...
                  docxName={`${selectedExam.title || "Speech-exam"}-${patient?.name || patientId}-Q${selectedExam.quarterOfYear}-${selectedExam.year}`}
        ...
        {isUploadPreviousModalOpen && ( // Modal for previous exam upload
          <div className={styles.modalOverlay}>
            <div className="modal-dialog modal-lg modal-dialog-centered">
              <div className={`modal-content ${styles.modalContent}`}>
                <div className="modal-header">
                  <h5 className="modal-title">Upload Previous Speech Exam</h5>
                  <button
                    type="button"
                    className="btn-close"
                ...
                <div className="modal-body p-0">
                  <DocxUploadFormSpeechExamPrevious
                    onSuccess={handleUploadSuccess}
