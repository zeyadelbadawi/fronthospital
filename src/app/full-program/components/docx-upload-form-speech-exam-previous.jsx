...
import ExistingSpeechExamsView from "./existing-speech-exams-view"
...
export default function DocxUploadFormSpeechExamPrevious({ onSuccess, onClose, defaultValues, onSwitchForm }) {
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      const existingExamCheck = await axiosInstance.get(
        `${process.env.NEXT_PUBLIC_API_URL}/speech/get-exams/${data.patientId}?quarter=${data.quarterOfYear}&year=${data.year}`,
      )
      ...
      const response = await axiosInstance.post(`${process.env.NEXT_PUBLIC_API_URL}/speech/upload-exam`, formData, {
        headers: {
