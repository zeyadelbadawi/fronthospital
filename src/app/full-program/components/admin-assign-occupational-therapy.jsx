...
const AdminAssignOccupationalTherapy = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      // Fetch OccupationalTherapy patients from PatientOccupationalTherapyAssignment collection
      console.log("Fetching Occupational Therapy patients from PatientOccupationalTherapyAssignment...")
      let patientsResponse
      try {
        patientsResponse = await axiosInstance.get("/OccupationalTherapy/patient-assignments", {
          params: { page: 1, search: "" },
        })
        console.log("OccupationalTherapy patients response:", patientsResponse.data)
      } catch (error) {
        console.error("OccupationalTherapy patients fetch failed:", error)
        setMessage("Failed to fetch OccupationalTherapy patients: " + (error.response?.data?.message || error.message))
        patientsResponse = { data: { assignments: [], patients: [] } }
      }

      // Fetch doctors in OccupationalTherapy department
      console.log("Fetching OccupationalTherapy doctors...")
      let doctorsResponse
      try {
        doctorsResponse = await axiosInstance.get("/doctor-student-assignment/doctors-by-department/OccupationalTherapy")
        console.log("OccupationalTherapy doctors response:", doctorsResponse.data)
      } catch (error) {
      ...
      // Fetch current doctor-student assignments for OccupationalTherapy department
      console.log("Fetching OccupationalTherapy doctor-student assignments...")
      let assignmentsResponse
      try {
        assignmentsResponse = await axiosInstance.get("/doctor-student-assignment/all-assignments", {
          params: { department: "OccupationalTherapy", page: 1, limit: 100 },
        })
        console.log("OccupationalTherapy assignments response:", assignmentsResponse.data)
      } catch (error) {
      ...
      // Set the data - patients from PatientOccupationalTherapyAssignment
      ...
      setDebugInfo(
        `Loaded: ${totalPatients} OccupationalTherapy patients, ${totalDoctors} doctors, ${assignedPatients} doctor assignments`,
      )

      if (doctorsData.length === 0) {
        setMessage(
          "No doctors found in OccupationalTherapy department. Please ensure doctors are properly assigned to the OccupationalTherapy department.",
        )
      }

      if (patientsData.length === 0) {
        setMessage(
          "No patients found in OccupationalTherapy assignments. Please ensure patients are assigned to the OccupationalTherapy department first.",
        )
      }
    } catch (error) {
  ...
  const handleAssignStudent = async (e) => {
    ...
    try {
      ...
      const response = await axiosInstance.post("/doctor-student-assignment/assign-student-to-doctor", {
        ...
        department: "OccupationalTherapy",
  ...
  if (loading && patients.length === 0) {
    return (
      <div className={styles.loadingContainer}>
        ...
        <p className={styles.loadingText}>Loading Occupational Therapy assignments...</p>
          ...
          <div className={styles.cardHeader}>
            <div className={styles.headerContent}>
              <div className={styles.titleSection}>
                <h2 className={styles.pageTitle}>
                  ...
                  Occupational Therapy Department - Admin Assignment
                </h2>
                <p className={styles.pageSubtitle}>Assign Occupational Therapy students to doctors and manage subscriptions</p>
              </div>
            ...
            <div className={styles.statsContainer}>
              <div className={styles.statItem}>
                ...
                <div className={styles.statLabel}>Total Occupational Therapy Patients</div>
              </div>
              <div className={styles.statItem}>
          ...
          <div className={styles.cardBody}>
            <div style={{ marginBottom: "2rem" }}>
              ...
              >
                ...
                Assign Occupational Therapy Student to Doctor
              </h2>
              <div
                style={{
                  height: "2px",
            ...
            <form onSubmit={handleAssignStudent} className={styles.editForm}>
              <div className={styles.formGrid}>
                <div className={styles.formGroup}>
                  <label className={styles.formLabel}>
                    ...
                    Select Occupational Therapy Patient *
                  </label>
                  <select
                    ...
                    required
                  >
                    <option value="">Choose an unassigned Occupational Therapy patient...</option>
                  ...
                  <p style={{ fontSize: "0.75rem", color: "#64748b", marginTop: "0.25rem", fontWeight: "500" }}>
                    📊 Unassigned Occupational Therapy patients: {getUnassignedPatients().length} / {patients.length}
                  </p>
                </div>
                ...
                <div className={styles.formGroup}>
                  <label className={styles.formLabel}>
                    ...
                    Select Occupational Therapy Doctor *
                  </label>
                  <select
                    ...
                    required
                  >
                    <option value="">Choose an Occupational Therapy doctor...</option>
                    {doctors.map((doctor) => (
                  ...
                  <p style={{ fontSize: "0.75rem", color: "#64748b", marginTop: "0.25rem", fontWeight: "500" }}>
                    👨‍⚕️ Available Occupational Therapy doctors: {doctors.length}
                  </p>
                </div>
              ...
              >
                ...
                >
                  ...
                  {isSubmitting ? "Assigning..." : "Assign Occupational Therapy Student to Doctor"}
                </button>
              </div>
            </form>
          </div>
...
export default AdminAssignOccupationalTherapy
