...
// Form validation schema for SpecialEducation Exam
...
export default function CloseQuarterFormSpecialExam({ onSuccess, onClose, defaultValues = {} }) {
  ...
  const onSubmit = async (data) => {
    ...
    try {
      // Call the new exam-specific endpoint (CHANGED FROM CLOSE-AND-GENERATE-NEXT-PLAN TO CLOSE-AND-GENERATE-NEXT-EXAM)
      const response = await axiosInstance.post(`${process.env.NEXT_PUBLIC_API_URL}/SpecialEducation/close-and-generate-next-exam`, {
      ...
      if (response.status !== 200) {
        throw new Error(response.data?.message || "Failed to close quarter and generate next exam") // Changed message
      }
      ...
      <form onSubmit={handleSubmit(onSubmit)}>
          ...
          <div className={styles.quarterInfoBox}>
            ...
            <div className={styles.quarterInfoContent}>
              ...
              <p className={styles.quarterInfoSubtitle}>
                Current Exam Quarter: Q{defaultValues.quarterOfYear} ({defaultValues.year}) {/* Changed text */}
              </p>
            </div>
          </div>

          <p className={styles.formText}>
            Select the quarter and year you want to close. A new exam for the next quarter will be automatically
            generated for this patient. {/* Changed text */}
          </p>
        </div>
        ...
        <div className={styles.formRow}>
          <div className={styles.warningNotice}>
            ...
            <div className={styles.warningContent}>
              ...
              current quarter's exam. {/* Changed text */}
            </div>
          </div>
        </div>
        ...
        >
          {isSubmitting ? (
            ...
            "Close Quarter & Generate Next Exam" // Changed button text
          )}
        </button>
      </form>
    </div>
