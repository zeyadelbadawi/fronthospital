...
const AssignPatientsToPhysicalTherapy = () => {
  ...
  const fetchAssignedPatients = async () => {
    try {
      const response = await axiosInstance.get(
        `${process.env.NEXT_PUBLIC_API_URL}/physicalTherapy/physical-therapy-assignments`,
      )
  ...
  const handleUnassignPatient = async (patientId) => {
    try {
      await axiosInstance.delete(
        `${process.env.NEXT_PUBLIC_API_URL}/physicalTherapy/unassign-from-physical/${patientId}`,
      )
