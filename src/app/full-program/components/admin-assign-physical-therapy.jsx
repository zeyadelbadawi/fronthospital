...
const AdminAssignPhysicalTherapy = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      // Fetch physicalTherapy patients from PatientPhysicalTherapyAssignment collection
      console.log("Fetching physical Therapy patients from PatientPhysicalTherapyAssignment...")
      let patientsResponse
      try {
        patientsResponse = await axiosInstance.get("/physicalTherapy/patient-assignments", {
          params: { page: 1, search: "" },
        })
        console.log("physicalTherapy patients response:", patientsResponse.data)
      } catch (error) {
        console.error("physicalTherapy patients fetch failed:", error)
        setMessage("Failed to fetch physicalTherapy patients: " + (error.response?.data?.message || error.message))
        patientsResponse = { data: { assignments: [], patients: [] } }
      }

      // Fetch doctors in physicalTherapy department
      console.log("Fetching physicalTherapy doctors...")
      let doctorsResponse
      try {
        doctorsResponse = await axiosInstance.get("/doctor-student-assignment/doctors-by-department/physicalTherapy")
        console.log("physicalTherapy doctors response:", doctorsResponse.data)
      } catch (error) {
      ...
      // Fetch current doctor-student assignments for physicalTherapy department
      console.log("Fetching physical Therapy doctor-student assignments...")
      let assignmentsResponse
      try {
        assignmentsResponse = await axiosInstance.get("/doctor-student-assignment/all-assignments", {
          params: { department: "physicalTherapy", page: 1, limit: 100 },
        })
        console.log("physicalTherapy assignments response:", assignmentsResponse.data)
      } catch (error) {
      ...
      // Set the data - patients from PatientPhysicalTherapyAssignment
      ...
      setDebugInfo(
        `Loaded: ${totalPatients} physical Therapy patients, ${totalDoctors} doctors, ${assignedPatients} doctor assignments`,
      )

      if (doctorsData.length === 0) {
        setMessage(
          "No doctors found in physical Therapy department. Please ensure doctors are properly assigned to the physical Therapy department.",
        )
      }

      if (patientsData.length === 0) {
        setMessage(
          "No patients found in physical Therapy assignments. Please ensure patients are assigned to the physical Therapy department first.",
        )
      }
    } catch (error) {
  ...
  if (loading && patients.length === 0) {
    return (
      <div className={styles.loadingContainer}>
        ...
        <p className={styles.loadingText}>Loading physical Therapy assignments...</p>
          ...
          <div className={styles.cardHeader}>
            <div className={styles.headerContent}>
              <div className={styles.titleSection}>
                <h2 className={styles.pageTitle}>
                  ...
                  physical Therapy Department - Admin Assignment
                </h2>
                <p className={styles.pageSubtitle}>Assign physical Therapy students to doctors and manage subscriptions</p>
              </div>
            ...
            <div className={styles.statsContainer}>
              <div className={styles.statItem}>
                ...
                <div className={styles.statLabel}>Total physical Therapy Patients</div>
              </div>
              <div className={styles.statItem}>
          ...
          <div className={styles.cardBody}>
            <div style={{ marginBottom: "2rem" }}>
              ...
              >
                ...
                Assign physical Therapy Student to Doctor
              </h2>
              <div
                style={{
                  height: "2px",
            ...
            <form onSubmit={handleAssignStudent} className={styles.editForm}>
              <div className={styles.formGrid}>
                <div className={styles.formGroup}>
                  <label className={styles.formLabel}>
                    ...
                    Select physical Therapy Patient *
                  </label>
                  <select
                    ...
                    required
                  >
                    <option value="">Choose an unassigned physical Therapy patient...</option>
                  ...
                  <p style={{ fontSize: "0.75rem", color: "#64748b", marginTop: "0.25rem", fontWeight: "500" }}>
                    📊 Unassigned physical Therapy patients: {getUnassignedPatients().length} / {patients.length}
                  </p>
                </div>
                ...
                <div className={styles.formGroup}>
                  <label className={styles.formLabel}>
                    ...
                    Select physical Therapy Doctor *
                  </label>
                  <select
                    ...
                    required
                  >
                    <option value="">Choose an physical Therapy doctor...</option>
                    {doctors.map((doctor) => (
                  ...
                  <p style={{ fontSize: "0.75rem", color: "#64748b", marginTop: "0.25rem", fontWeight: "500" }}>
                    👨‍⚕️ Available physical Therapy doctors: {doctors.length}
                  </p>
                </div>
              ...
              >
                ...
                >
                  ...
                  {isSubmitting ? "Assigning..." : "Assign physical Therapy Student to Doctor"}
                </button>
              </div>
            </form>
          </div>
...
export default AdminAssignPhysicalTherapy
