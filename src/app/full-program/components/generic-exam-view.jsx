...
export default function GenericExamView({ department, patientId, onBack }) {
  ...
  const determineActiveQuarter = async () => {
    ...
    try {
      const response = await axiosInstance.get(
        `${process.env.NEXT_PUBLIC_API_URL}/${config.endpoint}/get-exams/${patientId}?last=true`,
      )
  ...
  const fetchAllHistoricalExams = useCallback(async () => {
    ...
    try {
      const response = await axiosInstance.get(
        `${process.env.NEXT_PUBLIC_API_URL}/${config.endpoint}/exams/${patientId}`,
      )
  ...
  const handleSaveContent = async (documentContentBlob) => {
    ...
    try {
      ...
      formData.append("title", selectedExam.title || `${config.fullName} Exam for ${patient?.name || patientId}`)

      const endpoint =
        department === "occupational" || department === "special"
          ? `${process.env.NEXT_PUBLIC_API_URL}/${config.endpoint}/exam/${selectedExam._id}`
          : `${process.env.NEXT_PUBLIC_API_URL}/${config.endpoint.toLowerCase()}/exam/${selectedExam._id}`
            ...
            <div className={styles.planEditorCard}>
                ...
                <div className={styles.headerBottomRow}>
                  <div className={styles.studentInfo}>
                    <h3 className={styles.planTitle}>
                      ...
                      <span>{config.fullName} Exam for</span>
              ...
              <div className={styles.planBody}>
                <SyncfusionDocx
                  ...
                  docxName={`${selectedExam.title || `${config.displayName}-exam`}-${patient?.name || patientId}-Q${selectedExam.quarterOfYear}-${selectedExam.year}`}
        ...
        {isUploadPreviousModalOpen && (
          <div className={styles.modalOverlay}>
            <div className="modal-dialog modal-lg modal-dialog-centered">
              <div className={`modal-content ${styles.modalContent}`}>
                <div className="modal-header">
                  <h5 className="modal-title">Upload Previous {config.fullName} Exam</h5>
                  <button
                    type="button"
                    className="btn-close"
...
export function ABAExamView({ patientId, onBack }) {
  return <GenericExamView department="aba" patientId={patientId} onBack={onBack} />
}

export function SpeechExamView({ patientId, onBack }) {
  return <GenericExamView department="speech" patientId={patientId} onBack={onBack} />
}

export function PhysicalExamView({ patientId, onBack }) {
  return <GenericExamView department="physical" patientId={patientId} onBack={onBack} />
}

export function PsychotherapyExamView({ patientId, onBack }) {
  return <GenericExamView department="Psychotherapy" patientId={patientId} onBack={onBack} />
}

export function OccupationalExamView({ patientId, onBack }) {
  return <GenericExamView department="occupational" patientId={patientId} onBack={onBack} />
}

export function SpecialExamView({ patientId, onBack }) {
  return <GenericExamView department="special" patientId={patientId} onBack={onBack} />
}
