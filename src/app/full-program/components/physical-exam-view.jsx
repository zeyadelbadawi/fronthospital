...
import DocxUploadFormPhysicalExam from "./docx-upload-form-physical-exam"
import DocxUploadFormPhysicalExamPrevious from "./docx-upload-form-physical-exam-previous" // New import
import CloseQuarterFormPhysicalExam from "./close-quarter-form-physical-exam.jsx"
...
export default function PhysicalExamView({ patientId, onBack }) {
  ...
  const determineActiveQuarter = async () => {
    ...
    try {
      const response = await axiosInstance.get(
        `${process.env.NEXT_PUBLIC_API_URL}/physicalTherapy/get-exams/${patientId}?last=true`,
      )
  ...
  const fetchAllHistoricalExams = useCallback(async () => {
    ...
    setError(null)
    try {
      const response = await axiosInstance.get(`${process.env.NEXT_PUBLIC_API_URL}/physicalTherapy/exams/${patientId}`)
  ...
  const handleSaveContent = async (documentContentBlob) => {
    ...
    try {
      ...
      formData.append("title", selectedExam.title || `physicalTherapy Exam for ${patient?.name || patientId}`)
            ...
            <div className={styles.planEditorCard}>
                ...
                <div className={styles.headerBottomRow}>
                  <div className={styles.studentInfo}>
                    <h3 className={styles.planTitle}>
                      ...
                      <span>physicalTherapy Exam for</span>
              ...
              <div className={styles.planBody}>
                <SyncfusionDocx
                  ...
                  docxName={`${selectedExam.title || "physicalTherapy-exam"}-${patient?.name || patientId}-Q${selectedExam.quarterOfYear}-${selectedExam.year}`}
        ...
        {isUploadPreviousModalOpen && ( // Modal for previous exam upload
          <div className={styles.modalOverlay}>
            <div className="modal-dialog modal-lg modal-dialog-centered">
              <div className={`modal-content ${styles.modalContent}`}>
                <div className="modal-header">
                  <h5 className="modal-title">Upload Previous Physical Therapy Exam</h5>
                  <button
                    type="button"
                    className="btn-close"
                ...
                <div className="modal-body p-0">
                  <DocxUploadFormPhysicalExamPrevious
                    onSuccess={handleUploadSuccess}
