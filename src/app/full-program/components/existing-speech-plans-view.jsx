...
export default function ExistingSpeechPlansView({ patientId, onClose, onBackToUpload }) {
  ...
  const fetchPlans = useCallback(async () => {
    ...
    setError(null)
    try {
      const response = await axiosInstance.get(`${process.env.NEXT_PUBLIC_API_URL}/speech/get-plans/${patientId}`)
      ...
      console.error("Error fetching speech plans:", err)
  ...
  const handleDelete = async (planId) => {
    ...
    try {
      await axiosInstance.delete(`${process.env.NEXT_PUBLIC_API_URL}/speech/delete-plan/${planId}`)
      fetchPlans() // Refresh the list
    } catch (err) {
      console.error("Error deleting speech plan:", err)
  ...
  return (
    <div className={styles.existingDocumentsContainer}>
      ...
      {plans.length === 0 ? (
        <p className={styles.loadingMessage}>No existing speech plans found for this patient.</p>
      ) : (
