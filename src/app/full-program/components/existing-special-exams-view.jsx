...
export default function ExistingSpecialExamsView({ patientId, onClose, onBackToUpload }) {
  ...
  const fetchExams = useCallback(async () => {
    ...
    setError(null)
    try {
      const response = await axiosInstance.get(`${process.env.NEXT_PUBLIC_API_URL}/SpecialEducation/get-exams/${patientId}`)
      ...
      console.error("Error fetching SpecialEducation exams:", err)
  ...
  const handleDelete = async (examId) => {
    ...
    try {
      await axiosInstance.delete(`${process.env.NEXT_PUBLIC_API_URL}/SpecialEducation/delete-exam/${examId}`)
      fetchExams() // Refresh the list
    } catch (err) {
      console.error("Error deleting SpecialEducation exam:", err)
  ...
  return (
    <div className={styles.existingDocumentsContainer}>
      <div className={styles.existingDocumentsHeader}>
        <h4>
          Existing SpecialEducation Exams 
        </h4>
      ...
      {exams.length === 0 ? (
        <p className={styles.loadingMessage}>No existing SpecialEducation exams found for this patient.</p>
      ) : (
