...
import DocxUploadFormSpecial from "./docx-upload-form-special"
import DocxUploadFormSpecialPlanPrevious from "./docx-upload-form-special-plan-previous" // New import
import CloseQuarterFormSpecial from "./close-quarter-form-special"
...
export default function SpecialPlanView({ patientId, onBack }) {
  ...
  const determineActiveQuarter = async () => {
    ...
    try {
      const response = await axiosInstance.get(
        `${process.env.NEXT_PUBLIC_API_URL}/SpecialEducation/get-plans/${patientId}?last=true`,
      )
  ...
  const fetchAllHistoricalPlans = useCallback(async () => {
    ...
    setError(null)
    try {
      const response = await axiosInstance.get(`${process.env.NEXT_PUBLIC_API_URL}/SpecialEducation/plans/${patientId}`)
  ...
  const handleSaveContent = async (documentContentBlob) => {
    ...
    try {
      ...
      formData.append("title", selectedPlan.title || `SpecialEducation Plan for ${patient?.name || patientId}`)
            ...
            <div className={styles.planEditorCard}>
                ...
                <div className={styles.headerBottomRow}>
                  <div className={styles.studentInfo}>
                    <h3 className={styles.planTitle}>
                      ...
                      <span>SpecialEducation Plan for</span>
              ...
              <div className={styles.planBody}>
                <SyncfusionDocx
                  ...
                  docxName={`${selectedPlan.title || "SpecialEducation-plan"}-${patient?.name || patientId}-Q${selectedPlan.quarterOfYear}-${selectedPlan.year}`}
        ...
        {isUploadPreviousModalOpen && ( // Modal for previous plan upload
          <div className={styles.modalOverlay}>
            <div className="modal-dialog modal-lg modal-dialog-centered">
              <div className={`modal-content ${styles.modalContent}`}>
                <div className="modal-header">
                  <h5 className="modal-title">Upload Previous SpecialEducation Plan</h5>
                  <button
                    type="button"
                    className="btn-close"
                ...
                <div className="modal-body p-0">
                  <DocxUploadFormSpecialPlanPrevious
                    onSuccess={handleUploadSuccess}
