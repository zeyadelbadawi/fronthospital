...
import DocxUploadFormSpeech from "./docx-upload-form-speech"
import DocxUploadFormSpeechPlanPrevious from "./docx-upload-form-speech-plan-previous" // New import
import CloseQuarterFormSpeech from "./close-quarter-form-speech"
...
export default function SpeechPlanView({ patientId, onBack }) {
  ...
  const determineActiveQuarter = async () => {
    ...
    try {
      const response = await axiosInstance.get(
        `${process.env.NEXT_PUBLIC_API_URL}/speech/get-plans/${patientId}?last=true`,
      )
  ...
  const fetchAllHistoricalPlans = useCallback(async () => {
    ...
    setError(null)
    try {
      const response = await axiosInstance.get(`${process.env.NEXT_PUBLIC_API_URL}/speech/plans/${patientId}`)
  ...
  const handleSaveContent = async (documentContentBlob) => {
    ...
    try {
      ...
      formData.append("title", selectedPlan.title || `speech Plan for ${patient?.name || patientId}`)
            ...
            <div className={styles.planEditorCard}>
              ...
              <div className={styles.planBody}>
                <SyncfusionDocx
                  ...
                  docxName={`${selectedPlan.title || "speech-plan"}-${patient?.name || patientId}-Q${selectedPlan.quarterOfYear}-${selectedPlan.year}`}
        ...
        {isUploadPreviousModalOpen && ( // Modal for previous plan upload
          <div className={styles.modalOverlay}>
            <div className="modal-dialog modal-lg modal-dialog-centered">
              <div className={`modal-content ${styles.modalContent}`}>
                <div className="modal-header">
                  <h5 className="modal-title">Upload Previous Speech Plan</h5>
                  <button
                    type="button"
                    className="btn-close"
                ...
                <div className="modal-body p-0">
                  <DocxUploadFormSpeechPlanPrevious
                    onSuccess={handleUploadSuccess}
