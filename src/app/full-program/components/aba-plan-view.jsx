...
import DocxUploadFormABA from "./docx-upload-form-aba"
import DocxUploadFormABAPlanPrevious from "./docx-upload-form-aba-plan-previous" // New import
import CloseQuarterFormABA from "./close-quarter-form-aba"
...
export default function ABAPlanView({ patientId, onBack }) {
  ...
  const determineActiveQuarter = async () => {
    ...
    try {
      const response = await axiosInstance.get(
        `${process.env.NEXT_PUBLIC_API_URL}/aba/get-plans/${patientId}?last=true`,
      )
  ...
  const fetchAllHistoricalPlans = useCallback(async () => {
    ...
    setError(null)
    try {
      const response = await axiosInstance.get(`${process.env.NEXT_PUBLIC_API_URL}/aba/plans/${patientId}`)
  ...
  const handleSaveContent = async (documentContentBlob) => {
    ...
    try {
      ...
      formData.append("title", selectedPlan.title || `ABA Plan for ${patient?.name || patientId}`)
            ...
            <div className={styles.planEditorCard}>
              ...
              <div className={styles.planBody}>
                <SyncfusionDocx
                  ...
                  docxName={`${selectedPlan.title || "ABA-plan"}-${patient?.name || patientId}-Q${selectedPlan.quarterOfYear}-${selectedPlan.year}`}
        ...
        {isUploadPreviousModalOpen && ( // Modal for previous plan upload
          <div className={styles.modalOverlay}>
            <div className="modal-dialog modal-lg modal-dialog-centered">
              <div className={`modal-content ${styles.modalContent}`}>
                <div className="modal-header">
                  <h5 className="modal-title">Upload Previous ABA Plan</h5>
                  <button
                    type="button"
                    className="btn-close"
