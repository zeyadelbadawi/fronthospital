...
import DocxUploadFormOccupational from "./docx-upload-form-occupational"
import DocxUploadFormOccupationalPlanPrevious from "./docx-upload-form-occupational-plan-previous" // New import
import CloseQuarterFormOccupational from "./close-quarter-form-occupational"
...
export default function OccupationalPlanView({ patientId, onBack }) {
  ...
  const determineActiveQuarter = async () => {
    ...
    try {
      const response = await axiosInstance.get(
        `${process.env.NEXT_PUBLIC_API_URL}/OccupationalTherapy/get-plans/${patientId}?last=true`,
      )
  ...
  const fetchAllHistoricalPlans = useCallback(async () => {
    ...
    setError(null)
    try {
      const response = await axiosInstance.get(`${process.env.NEXT_PUBLIC_API_URL}/OccupationalTherapy/plans/${patientId}`)
  ...
  const handleSaveContent = async (documentContentBlob) => {
    ...
    try {
      ...
      formData.append("title", selectedPlan.title || `Occupational Therapy Plan for ${patient?.name || patientId}`)

      const response = await axiosInstance.put(
        `${process.env.NEXT_PUBLIC_API_URL}/${department}/plan/${selectedPlan._id}`,
        formData,
        {
            ...
            <div className={styles.planEditorCard}>
                ...
                <div className={styles.headerBottomRow}>
                  <div className={styles.studentInfo}>
                    <h3 className={styles.planTitle}>
                      ...
                      <span>Occupational Therapy Plan for</span>
              ...
              <div className={styles.planBody}>
                <SyncfusionDocx
                  ...
                  docxName={`${selectedPlan.title || "OccupationalTherapy-plan"}-${patient?.name || patientId}-Q${selectedPlan.quarterOfYear}-${selectedPlan.year}`}
        ...
        {isUploadPreviousModalOpen && ( // Modal for previous plan upload
          <div className={styles.modalOverlay}>
            <div className="modal-dialog modal-lg modal-dialog-centered">
              <div className={`modal-content ${styles.modalContent}`}>
                <div className="modal-header">
                  <h5 className="modal-title">Upload Previous Occupational Therapy Plan</h5>
                  <button
                    type="button"
                    className="btn-close"
                ...
                <div className="modal-body p-0">
                  <DocxUploadFormOccupationalPlanPrevious
                    onSuccess={handleUploadSuccess}
