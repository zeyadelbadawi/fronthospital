...
const AdminAssignABA = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      try {
        patientsResponse = await axiosInstance.get("/aba/patient-assignments", {
          params: { page: 1, search: "" },
        })
        console.log("ABA patients response:", patientsResponse.data)
      } catch (error) {
        console.error("ABA patients fetch failed:", error)
        setMessage("Failed to fetch ABA patients: " + (error.response?.data?.message || error.message))
        patientsResponse = { data: { assignments: [], patients: [] } }
      }

      // Fetch doctors in ABA department
      console.log("Fetching ABA doctors...")
      let doctorsResponse
      try {
        doctorsResponse = await axiosInstance.get("/doctor-student-assignment/doctors-by-department/ABA")
        console.log("ABA doctors response:", doctorsResponse.data)
      } catch (error) {
      ...
      // Fetch current doctor-student assignments for ABA department
      console.log("Fetching ABA doctor-student assignments...")
      let assignmentsResponse
      try {
        assignmentsResponse = await axiosInstance.get("/doctor-student-assignment/all-assignments", {
          params: { department: "ABA", page: 1, limit: 100 },
        })
        console.log("ABA assignments response:", assignmentsResponse.data)
      } catch (error) {
      ...
      // Set the data - patients from PatientABAAssignment
      ...
      if (doctorsData.length === 0) {
        setMessage(
          "No doctors found in ABA department. Please ensure doctors are properly assigned to the ABA department.",
        )
      }

      if (patientsData.length === 0) {
        setMessage(
          "No patients found in ABA assignments. Please ensure patients are assigned to the ABA department first.",
        )
      }
    } catch (error) {
          ...
          <div className={styles.cardHeader}>
            <div className={styles.headerContent}>
              <div className={styles.titleSection}>
                <h2 className={styles.pageTitle}>
                  ...
                  ABA Department - Admin Assignment
                </h2>
                <p className={styles.pageSubtitle}>Assign ABA students to doctors and manage subscriptions</p>
              </div>
            ...
            <div className={styles.statsContainer}>
              <div className={styles.statItem}>
                ...
                <div className={styles.statLabel}>Total ABA Patients</div>
              </div>
              <div className={styles.statItem}>
            ...
            <form onSubmit={handleAssignStudent} className={styles.editForm}>
              <div className={styles.formGrid}>
                <div className={styles.formGroup}>
                  ...
                  <select
                    ...
                    required
                  >
                    <option value="">Choose an unassigned ABA patient...</option>
                  ...
                  <p style={{ fontSize: "0.75rem", color: "#64748b", marginTop: "0.25rem", fontWeight: "500" }}>
                    📊 Unassigned ABA patients: {getUnassignedPatients().length} / {patients.length}
                  </p>
                </div>
                ...
                <div className={styles.formGroup}>
                  ...
                  <select
                    ...
                    required
                  >
                    <option value="">Choose an ABA doctor...</option>
                    {doctors.map((doctor) => (
                  ...
                  <p style={{ fontSize: "0.75rem", color: "#64748b", marginTop: "0.25rem", fontWeight: "500" }}>
                    👨‍⚕️ Available ABA doctors: {doctors.length}
                  </p>
                </div>
              ...
              >
                ...
                >
                  ...
                  {isSubmitting ? "Assigning..." : "Assign ABA Student to Doctor"}
                </button>
              </div>
            </form>
          </div>
