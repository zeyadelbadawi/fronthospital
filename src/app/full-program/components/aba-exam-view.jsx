...
import DocxUploadFormABAExam from "./docx-upload-form-aba-exam"
import DocxUploadFormABAExamPrevious from "./docx-upload-form-aba-exam-previous" // New import
import CloseQuarterFormABAExam from "./close-quarter-form-aba-exam.jsx"
...
export default function ABAExamView({ patientId, onBack }) {
  ...
  const determineActiveQuarter = async () => {
    ...
    try {
      const response = await axiosInstance.get(
        `${process.env.NEXT_PUBLIC_API_URL}/aba/get-exams/${patientId}?last=true`,
      )
  ...
  const fetchAllHistoricalExams = useCallback(async () => {
    ...
    setError(null)
    try {
      const response = await axiosInstance.get(`${process.env.NEXT_PUBLIC_API_URL}/aba/exams/${patientId}`)
  ...
  const handleSaveContent = async (documentContentBlob) => {
    ...
    try {
      ...
      formData.append("title", selectedExam.title || `ABA Exam for ${patient?.name || patientId}`)
            ...
            <div className={styles.planEditorCard}>
              ...
              <div className={styles.planBody}>
                <SyncfusionDocx
                  ...
                  docxName={`${selectedExam.title || "ABA-exam"}-${patient?.name || patientId}-Q${selectedExam.quarterOfYear}-${selectedExam.year}`}
        ...
        {isUploadPreviousModalOpen && ( // Modal for previous exam upload
          <div className={styles.modalOverlay}>
            <div className="modal-dialog modal-lg modal-dialog-centered">
              <div className={`modal-content ${styles.modalContent}`}>
                <div className="modal-header">
                  <h5 className="modal-title">Upload Previous ABA Exam</h5>
                  <button
                    type="button"
                    className="btn-close"
