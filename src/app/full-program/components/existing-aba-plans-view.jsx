...
export default function ExistingABAPlansView({ patientId, onClose, onBackToUpload }) {
  ...
  const fetchPlans = useCallback(async () => {
    ...
    setError(null)
    try {
      const response = await axiosInstance.get(`${process.env.NEXT_PUBLIC_API_URL}/aba/get-plans/${patientId}`)
      ...
      console.error("Error fetching ABA plans:", err)
  ...
  const handleDelete = async (planId) => {
    ...
    try {
      await axiosInstance.delete(`${process.env.NEXT_PUBLIC_API_URL}/aba/delete-plan/${planId}`)
      fetchPlans() // Refresh the list
    } catch (err) {
      console.error("Error deleting ABA plan:", err)
  ...
  return (
    <div className={styles.existingDocumentsContainer}>
      ...
      {plans.length === 0 ? (
        <p className={styles.loadingMessage}>No existing ABA plans found for this patient.</p>
      ) : (
