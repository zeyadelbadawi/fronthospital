...
export default function ExistingOccupationalPlansView({ patientId, onClose, onBackToUpload }) {
  ...
  const fetchPlans = useCallback(async () => {
    ...
    setError(null)
    try {
      const response = await axiosInstance.get(`${process.env.NEXT_PUBLIC_API_URL}/OccupationalTherapy/get-plans/${patientId}`)
      ...
      console.error("Error fetching Occupational Therapy plans:", err)
  ...
  const handleDelete = async (planId) => {
    ...
    try {
      await axiosInstance.delete(`${process.env.NEXT_PUBLIC_API_URL}/OccupationalTherapy/delete-plan/${planId}`)
      fetchPlans() // Refresh the list
    } catch (err) {
      console.error("Error deleting Occupational Therapy plan:", err)
  ...
  return (
    <div className={styles.existingDocumentsContainer}>
      <div className={styles.existingDocumentsHeader}>
        <h4>
          Existing Occupational Therapy Plans
        </h4>
      ...
      {plans.length === 0 ? (
        <p className={styles.loadingMessage}>No existing Occupational Therapy plans found for this patient.</p>
      ) : (
