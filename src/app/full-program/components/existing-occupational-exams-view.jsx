...
export default function ExistingOccupationalExamsView({ patientId, onClose, onBackToUpload }) {
  ...
  const fetchExams = useCallback(async () => {
    ...
    setError(null)
    try {
      const response = await axiosInstance.get(`${process.env.NEXT_PUBLIC_API_URL}/OccupationalTherapy/get-exams/${patientId}`)
      ...
      console.error("Error fetching Occupational Therapy exams:", err)
  ...
  const handleDelete = async (examId) => {
    ...
    try {
      await axiosInstance.delete(`${process.env.NEXT_PUBLIC_API_URL}/OccupationalTherapy/delete-exam/${examId}`)
      fetchExams() // Refresh the list
    } catch (err) {
      console.error("Error deleting Occupational Therapy exam:", err)
  ...
  return (
    <div className={styles.existingDocumentsContainer}>
      <div className={styles.existingDocumentsHeader}>
        <h4>
          Existing Occupational Therapy Exams 
        </h4>
      ...
      {exams.length === 0 ? (
        <p className={styles.loadingMessage}>No existing Occupational Therapy exams found for this patient.</p>
      ) : (
