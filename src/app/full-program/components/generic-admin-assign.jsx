...
if (!config) {
  ...
  const fetchData = async () => {
    try {
      ...
      console.log(`Fetching ${config.name} patients from Patient${config.name}Assignment...`)
      let patientsResponse
      try {
        patientsResponse = await axiosInstance.get(`/${config.endpoint}/patient-assignments`, {
          params: { page: 1, search: "" },
        })
        console.log(`${config.name} patients response:`, patientsResponse.data)
      } catch (error) {
        console.error(`${config.name} patients fetch failed:`, error)
        setMessage(`Failed to fetch ${config.name} patients: ` + (error.response?.data?.message || error.message))
      ...
      if (patientsData.length === 0) {
        setMessage(
          `No patients found in ${config.displayName} assignments. Please ensure patients are assigned to the ${config.displayName} department first.`,
        )
      }
    } catch (error) {
          ...
          <div className={styles.cardHeader}>
            ...
            <div className={styles.statsContainer}>
              <div className={styles.statItem}>
                ...
                <div className={styles.statLabel}>Total {config.displayName} Patients</div>
              </div>
              <div className={styles.statItem}>
              ...
              <div className={styles.formGrid}>
                <div className={styles.formGroup}>
                  <label className={styles.formLabel}>
                    ...
                    Select {config.displayName} Patient *
                  </label>
                  <select
