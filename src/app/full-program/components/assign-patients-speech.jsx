...
const AssignPatientsToSpeech = () => {
  ...
  const fetchAssignedPatients = async () => {
    try {
      const response = await axiosInstance.get(`${process.env.NEXT_PUBLIC_API_URL}/speech/Speech-assignments`)
  ...
  const handleAssignPatient = async (patientId) => {
    ...
    try {
      ...
      alert("Student assigned to speech department successfully!")
    } catch (error) {
      console.error("Error assigning Student:", error)
      alert("Error assigning Student to speech department")
    }
  }

  const handleUnassignPatient = async (patientId) => {
    try {
      await axiosInstance.delete(`${process.env.NEXT_PUBLIC_API_URL}/speech/unassign-from-Speech/${patientId}`)
      setAssignedPatients(assignedPatients.filter((id) => id !== patientId))
      alert("Student unassigned from speech department successfully!")
    } catch (error) {
      console.error("Error unassigning Student:", error)
      alert("Error unassigning Student from Speech department")
    }
  }
        ...
        <div className={styles.cardHeader}>
          <div className={styles.headerContent}>
            <div className={styles.headerLeft}>
              ...
              <h2 className={styles.pageTitle}>Assign Students to Speech Department</h2>
            </div>
