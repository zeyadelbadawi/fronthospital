...
import ExistingOccupationalPlansView from "./existing-occupational-plans-view"

// Form validation schema for OccupationalTherapy
...
export default function DocxUploadFormOccupationalPlanPrevious({ onSuccess, onClose, defaultValues, onSwitchForm }) {
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      const existingPlanCheck = await axiosInstance.get(
        `${process.env.NEXT_PUBLIC_API_URL}/OccupationalTherapy/get-plans/${data.patientId}?quarter=${data.quarterOfYear}&year=${data.year}`,
      )

      if (
        existingPlanCheck.data.plans &&
        (Array.isArray(existingPlanCheck.data.plans)
          ? existingPlanCheck.data.plans.length > 0
          : existingPlanCheck.data.plans)
      ) {
      ...
      const response = await axiosInstance.post(`${process.env.NEXT_PUBLIC_API_URL}/OccupationalTherapy/upload-plan`, formData, {
        headers: {
        ...
        <div className={styles.buttonGroup}>
          ...
          >
            {isSubmitting ? (
              ...
              "Upload Previous Occupational Therapy Plan"
            )}
          </button>
          <button
            type="button"
          ...
          {onSwitchForm && (
            <button type="button" className={`${styles.secondaryButton}`} onClick={() => onSwitchForm("exam")}>
              Switch to Exam Upload
            </button>
          )}
        </div>
      </form>
