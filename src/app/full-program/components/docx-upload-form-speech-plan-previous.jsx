...
import ExistingSpeechPlansView from "./existing-speech-plans-view"

// Form validation schema for speech
...
export default function DocxUploadFormSpeechPlanPrevious({ onSuccess, onClose, defaultValues, onSwitchForm }) {
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      const existingPlanCheck = await axiosInstance.get(
        `${process.env.NEXT_PUBLIC_API_URL}/speech/get-plans/${data.patientId}?quarter=${data.quarterOfYear}&year=${data.year}`,
      )
      ...
      const response = await axiosInstance.post(`${process.env.NEXT_PUBLIC_API_URL}/speech/upload-plan`, formData, {
        headers: {
