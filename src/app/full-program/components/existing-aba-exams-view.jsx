...
export default function ExistingABAExamsView({ patientId, onClose, onBackToUpload }) {
  ...
  const fetchExams = useCallback(async () => {
    ...
    setError(null)
    try {
      const response = await axiosInstance.get(`${process.env.NEXT_PUBLIC_API_URL}/aba/get-exams/${patientId}`)
      ...
      console.error("Error fetching ABA exams:", err)
  ...
  const handleDelete = async (examId) => {
    ...
    try {
      await axiosInstance.delete(`${process.env.NEXT_PUBLIC_API_URL}/aba/delete-exam/${examId}`)
      fetchExams() // Refresh the list
    } catch (err) {
      console.error("Error deleting ABA exam:", err)
  ...
  return (
    <div className={styles.existingDocumentsContainer}>
      ...
      {exams.length === 0 ? (
        <p className={styles.loadingMessage}>No existing ABA exams found for this patient.</p>
      ) : (
