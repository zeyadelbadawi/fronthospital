...
export default function ExistingPhysicalPlansView({ patientId, onClose, onBackToUpload }) {
  ...
  const fetchPlans = useCallback(async () => {
    ...
    setError(null)
    try {
      const response = await axiosInstance.get(`${process.env.NEXT_PUBLIC_API_URL}/physicalTherapy/get-plans/${patientId}`)
      ...
      console.error("Error fetching physicalTherapy plans:", err)
  ...
  const handleDelete = async (planId) => {
    ...
    try {
      await axiosInstance.delete(`${process.env.NEXT_PUBLIC_API_URL}/physicalTherapy/delete-plan/${planId}`)
      fetchPlans() // Refresh the list
    } catch (err) {
      console.error("Error deleting physicalTherapy plan:", err)
  ...
  return (
    <div className={styles.existingDocumentsContainer}>
      <div className={styles.existingDocumentsHeader}>
        <h4>
          Existing physicalTherapy Plans
        </h4>
      ...
      {plans.length === 0 ? (
        <p className={styles.loadingMessage}>No existing physicalTherapy plans found for this patient.</p>
      ) : (
