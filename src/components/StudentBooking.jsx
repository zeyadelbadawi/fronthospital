...
import axiosInstance from "@/helper/axiosSetup"; // Import axios instance for API calls
      ...
      <div className={styles.ruknDocumentImportLoader}>
        <div className={styles.ruknDocumentImportContent}>
          ...
          <h4 className={styles.ruknDocumentImportTitle}>
            {
              "تحميل محرر المستندات..." /* Arabic */ ||
                "Loading Document Editor..." /* English */
            }
          </h4>
...
const StudentBooking = ({
    ...
    () => [
      {
        ...
        label: (
          <>
            <School size={18} className={styles.iconInline} />{" "}
      ...
      },
      {
        ...
        label: (
          <>
            ...
            {language === "ar"
              ? "تقييم الحزمة الكاملة"
              : "Full Package Evaluation"}
            {hasActiveFullProgram &&
              (language === "ar" ? " (نشط بالفعل)" : " (Already Active)")}
          </>
        ),
        textLabel:
          language === "ar"
      ...
      },
      {
        ...
        label: (
          <>
            <User size={18} className={styles.iconInline} />{" "}
            {language === "ar" ? "جلسة واحدة" : "Single Session"}
          </>
        ),
  ...
  const serviceOptions = useMemo(
    () => [
      ...
      },
      {
        ...
        label: (
          <>
            <Brain size={18} className={styles.iconInline} />{" "}
            {language === "ar" ? "تحليل السلوك التطبيقي" : "ABA"}
          </>
        ),
        price: 300,
      },
      ...
      },
      {
        ...
        label: (
          <>
            <Mic size={18} className={styles.iconInline} />{" "}
            {language === "ar" ? "علاج النطق" : "Speech"}
          </>
        ),
        price: 230,
      },
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        const [planResponse, programResponse] = await Promise.allSettled([
          axiosInstance.get(
            `${process.env.NEXT_PUBLIC_API_URL}/authentication/patient-case-study/${patientId}`
          ), // Updated endpoint for case study
          axiosInstance.get(
    ...
    (step) => {
      ...
      switch (step) {
        ...
        case 1:
          ...
          if (!selectedTime) {
            errors.selectedTime =
              language === "ar" ? "يرجى اختيار الوقت" : "Please select a time";
          }
          if (!description.trim()) {
            errors.description =
              language === "ar"
        ...
        case 2:
          ...
          if (!plan?.hasCaseStudy) {
            errors.document =
              language === "ar"
                ? "يجب عليك إنهاء مستند الوورد أولاً"
                : "You must finish your word document first";
          }
          break;
        default:
          break;
    ...
    async (patientId, description) => {
      try {
        ...
        const assignmentData = {
          patientId: patientId,
          notes:
            description || `School evaluation assignment for ${patientName}`,
        };
        const response = await axiosInstance.post(
    ...
    try {
      ...
      if (response.status === 200) {
        ...
        if (moneyResponse.status === 200) {
          ...
          resetFormFields();
          console.log("TT========>", {
            pationt: { patientId, patientName, patientEmail },
            message: programPayload.programType,
            date: programPayload.date,
            time: programPayload.time,
          });
          ...
          alert(
            language === "ar"
              ? `عذراً، هذا الموعد محجوز بالفعل. يرجى اختيار وقت آخر.\n\nالموعد المتضارب: ${
                  errorData.conflictingAppointment?.time || "غير محدد"
                }`
    ...
    () => [
      {
        number: 1,
        title:
          language === "ar" ? "اختر نوع التقييم" : "Select Evaluation Type",
  ...
  return (
    <div
      className={`${styles.ruknBookingWizard} ${
          ...
          <div className={styles.ruknCardContent}>
                  ...
                  <div className={styles.ruknFormGroup}>
                    ...
                    {isLoadingAvailability && (
                      <div className={styles.ruknAvailabilityLoading}>
                        ...
                        <span>
                          {language === "ar"
                            ? "جارٍ التحقق من المواعيد المتاحة..."
                    ...
                    {selectedDay && !isLoadingAvailability && (
                      <div className={styles.ruknTimeSlotsGrid}>
                        {console.log(
                          "Rendering time slots grid with bookedSlots:",
                          bookedSlots
                        )}{" "}
                  ...
                  {programData.programType === "full_program" ? (
                    <>
                      ...
                      <div className={styles.ruknPaymentRow}>
                        <span>
                          {language === "ar"
                            ? "المتبقي (بعد الاستشارة):"
                            : "Remaining (After Consultation):"}
                        </span>
                        <span>4,000 EGP</span>
                      </div>
                      <div className={styles.ruknPaymentTotal}>
                        <span>
                          {language === "ar" ? "الدفع الآن:" : "Paying Now:"}
                        </span>
                        <span>1,000 EGP</span>
                      </div>
            ...
            {currentStep === 4 && (
              <div className={styles.ruknSuccessContainer}>
                ...
                <h3 className={styles.ruknSuccessTitle}>
                ...
                {assignmentResults && (
                  <div className={styles.ruknAssignmentFinalResults}>
                    ...
                    <p>
                      {language === "ar"
                        ? "لقد تم تعيينك تلقائيًا إلى"
                        : "You have been automatically assigned to"}{" "}
                      {assignmentResults.totalAssigned}{" "}
                      {language === "ar" ? "خدمة (خدمات)." : "service(s)."}
                    </p>
