...
const PublicProfileVolunteer = () => {
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (!userRole || !userId || userRole !== 'volunteer') {
        alert('You must be a volunteer to access this page');
        router.push('/sign-in');  // Redirect if user is not a volunteer
        return;
      }

      setVolunteerId(userId);  // Store the volunteer ID from the token

      // Fetch volunteer data
      const fetchVolunteer = async () => {
        try {
          const response = await axios.get(`${process.env.NEXT_PUBLIC_API_URL}/authentication/volunteer/${userId}`, {
  ...
  // Loading state until volunteer data is fetched
...
export default PublicProfileVolunteer;
