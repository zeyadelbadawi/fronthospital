...
const PublicProfileDoctor = () => {
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (!userRole || !userId || userRole !== 'doctor') {
        ...
        router.push('/sign-in');  // Redirect if user is not a doctor
        return;
      }

      setDoctorId(userId);  // Store the doctor ID from the token
  ...
  // Loading state until doctor data is fetched
              ...
              <div className="tab-pane fade show active" id="pills-edit-profile" role="tabpanel" aria-labelledby="pills-edit-profile-tab" tabIndex={0}>
                <form onSubmit={handleSubmit}>
                  <div className="mb-20">
                    ...
                    <input type="text" className="form-control radius-8" id="username" value={username} onChange={(e) => setUsername(e.target.value)} placeholder="Enter Username" />
                  </div>
                  <div className="mb-20">
