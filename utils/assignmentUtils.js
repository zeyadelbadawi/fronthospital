...
export const formatServiceName = (serviceName) => {
  ...
  return serviceMap[serviceName] || serviceName.replace(/_/g, " ").replace(/\b\w/g, (l) => l.toUpperCase())
}
...
export const validateAppointmentData = (data) => {
  ...
  if (data.evaluationType === "single_session" && (!data.services || data.services.length === 0)) {
    errors.services = "At least one service must be selected for single session"
  }
